...
{
    //
    private $obj_info = ['name' => 'currencyexchange', 'routing' => 'admin.controller', 'title' => 'Currencyexchange', 'icon' => '<i class="text-muted fas fa-money-check-alt"></i>'];
    public $args;
    ...
    private $tablename;
    private $columns = [];
    private $fprimarykey = 'exchange_id';
    ...
    public function __construct(array $args = [])
    {
        ...
        // dd($args['userinfo']);
        $this->obj_info['title'] =  __('dev.daily_exchange')." - ".__('table.t_dolla');
        ...
        $this->args = $args;
        $this->model = new Currencyexchange;
    ...
    public function listingModel()
    {
        #DEFIND MODEL#
        return $this->model
            ->leftJoin('users', 'users.id', 'tblcurrency_exchange.blongto')
            ->select(
                \DB::raw($this->fprimarykey . ",tblcurrency_exchange.buy_in,tblcurrency_exchange.sold_out,tblcurrency_exchange.create_date,
                         tblcurrency_exchange.update_date,tblcurrency_exchange.status,users.name As username"),
            )->whereRaw('tblcurrency_exchange.trash <> "yes"');
    } /*../function..*/
    public function listingtrash()
    {
        ...
        return $this->model
            ...
            )->whereRaw('tblcurrency_exchange.trash <> "no"');
    } /*../function..*/
    ...
    public function sfp($request, $results)
    {
        ...
        $appends = [];
        $querystr = [];
        if ($request->has('txtcurrencyexchange') && !empty($request->input('txtcurrencyexchange'))) {
            $qry = $request->input('txtcurrencyexchange');
            $results = $results->where(function ($query) use ($qry) {
                $query->whereRaw("tblcurrency_exchange.buy_in like '%" . $qry . "%'")
                    ->orwhereRaw("tblcurrency_exchange.sold_out like '%" . $qry . "%'")
    ...
    public function index(Request $request, $condition = [], $setting = [])
    {
        ...
        $currencyexchange = $default['currencyexchange'];
        //dd('aaa');
        ...
        return view('app.' . $this->obj_info['name'] . '.index')
            ...
            ->with(['currencyexchange' => $currencyexchange])
            ->with($sfp)
            ->with($setting);
    }
    ...
    public function validator($request, $isupdate = false)
    {
        ...
        $rules['buy_in'] = ['required'];
        $rules['sold_out'] = ['required'];
        $validatorMessages = [
    ...
    function proceed_store($request, $data, $obj_info)
    {
        ...
        // dd($save_status);
        if ($save_status) {
            //$request->file('images')->storeAs('currencyexchange', $data['tableData']['image_url']);
            ...
            return response()
                ->json(
                    [
                        ...
                        "callback" => $callback,
                        "data" => [
                            $this->fprimarykey => $data['exchange_id'],
                            'id' => $data['exchange_id']
                        ]
                    ],
                    200
    ...
    public function restore(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('exchange_id', $editid)->update(["trash" => "no"]);
    ...
    public function totrash(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('exchange_id', $editid)->update(["trash" => "yes"]);
    ...
    public function disable(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('exchange_id', $editid)->update(["status" => "no"]);
    ...
    public function enable(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('exchange_id', $editid)->update(["status" => "yes"]);
    ...
    public function status(Request $request, $id = 0)
    {
        ...
        $trash = $this->model->where('exchange_id', $editid)->update(["status" => "" . $status]);
        if ($trash && $status == "yes") {
            return response()
                ->json(
                    [
