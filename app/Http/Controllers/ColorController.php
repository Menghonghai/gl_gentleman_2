...
{
    //
    private $obj_info = ['name' => 'color', 'routing' => 'admin.controller', 'title' => 'Color', 'icon' => '<i class="text-muted fas fa-adjust"></i>'];
    public $args;
    ...
    public function listingModel()
    {
        ...
        return $this->model
            ...
            ->select(
                ...
                tblcolors.code,tblcolors.update_date,tblcolors.status,users.name As username"),
    ...
    public function sfp($request, $results)
    {
        ...
        if ($request->has('txtcolor') && !empty($request->input('txtcolor'))) {
            ...
            $results = $results->where(function ($query) use ($qry) {
                ...
                ->orwhereRaw("tblcolors.code like '%" . $qry . "%'")
    ...
    public function status(Request $request, $id = 0)
    {
        ...
        $trash = $this->model->where('color_id', $editid)->update(["status" => "" . $status]);
        if ($trash && $status == "yes") {
            return response()
                ->json(
                    [
