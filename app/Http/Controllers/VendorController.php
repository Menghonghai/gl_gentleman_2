...
{
    //
    private $obj_info = ['name' => 'vendor', 'routing' => 'admin.controller', 'title' => 'Vendor', 'icon' => '<i class="text-muted fas fa-industry"></i>'];
    public $args;
    ...
    public function listingModel()
    {
        ...
        return $this->model
            ...
            ->select(
                ...
                tblvendors.code,tblvendors.image_url,tblvendors.type,tblvendors.update_date,tblvendors.status,users.name As username"),
    ...
    public function sfp($request, $results)
    {
        ...
        if ($request->has('txtvendor') && !empty($request->input('txtvendor'))) {
            ...
            $results = $results->where(function ($query) use ($qry) {
                ...
                ->orwhereRaw("tblvendors.code like '%" . $qry . "%'")
        ...
        if ($request->has('create_date') && !empty($request->input('create_date'))) {
            ...
            $date = date_query($request, 'tblvendors.create_date', 'create_date', $_date[0], last($_date));
    ...
    public function status(Request $request, $id = 0)
    {
        ...
        $trash = $this->model->where('vendor_id', $editid)->update(["status" => "" . $status]);
        if ($trash && $status == "yes") {
            return response()
                ->json(
                    [
