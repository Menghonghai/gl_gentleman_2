...
{
    //
    private $obj_info = ['name' => 'bankcorporate', 'routing' => 'admin.controller', 'title' => 'Bank Corporate', 'icon' => '<i class="text-muted fab fa-cc-visa"></i>'];
    public $args;
    ...
    public function __construct(array $args = [])
    {
        ...
        // dd($args['userinfo']);
        $this->obj_info['title'] =  __('dev.bank_corporate');
    ...
    public function listingModel()
    {
        #DEFIND MODEL#
        return $this->model
            ->leftJoin('users', 'users.id', 'tblbank_corporate.blongto')
            ->select(
                \DB::raw($this->fprimarykey . ",JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".name,'$." . $this->dflang[0] . "')) AS text,
                JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".account_number,'$.en')) AS account_num_usd,
                JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".account_number,'$.kh')) AS account_num_khr,
                tblbank_corporate.code,tblbank_corporate.create_date,tblbank_corporate.image_url,tblbank_corporate.update_date,tblbank_corporate.status,users.name As username"),

            )->whereRaw('tblbank_corporate.trash <> "yes"');
    } /*../function..*/
    public function listingtrash()
    {
        ...
        return $this->model
            ...
            )->whereRaw('tblbank_corporate.trash <> "no"');
    } /*../function..*/
    ...
    public function sfp($request, $results)
    {
        ...
        if ($request->has('txtbankcorporate') && !empty($request->input('txtbankcorporate'))) {
            ...
            $results = $results->where(function ($query) use ($qry) {
                $query->whereRaw("JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".name,'$." . $this->dflang[0] . "')) like '%" . $qry . "%'")
                    ->orwhereRaw("JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".account_number,'$.en')) like '%" . $qry . "%'")
                    ->orwhereRaw("JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".account_number,'$.kh')) like '%" . $qry . "%'")
                    ->orwhereRaw("tblbank_corporate.code like '%" . $qry . "%'")
        ...
        if ($request->has('status') && !empty($request->input('status'))) {
            ...
            $results = $results->where("tblbank_corporate.status", $qry);
            array_push($querystr, 'tblbank_corporate.status=' . $qry);
            $appends = array_merge($appends, ['tblbank_corporate.status' => $qry]);
        }
    ...
    public function setinfo($request, $isupdate = false)
    {
        ...
        $account = toTranslate($request, 'account', 0, true);
    ...
    public function edit(Request $request, $id = 0)
    {
        ...
        $account_number = json_decode($input['account_number'], true);
        $sumit_route = url_builder(
            $this->obj_info['routing'],
    ...
    public function status(Request $request, $id = 0)
    {
        ...
        $trash = $this->model->where('bank_id', $editid)->update(["status" => "" . $status]);
        if ($trash && $status == "yes") {
            return response()
                ->json(
                    [
