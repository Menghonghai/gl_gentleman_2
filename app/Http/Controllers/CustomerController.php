...
{
    //
    private $obj_info = ['name' => 'customer', 'routing' => 'admin.controller', 'title' => 'Customer', 'icon' => '<i class="text-muted fas fa-address-card"></i>'];
    public $args;
    ...
    public function listingModel()
    {
        ...
        return $this->model
            ...
            ->select(
                \DB::raw($this->fprimarykey . ",JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".name,'$." . $this->dflang[0] . "')) AS text,tblcustomers.code,tblcustomers.gender,tblcustomers.phone_number,tblcustomers.email,
        ...
        if ($request->has('txtcustomer') && !empty($request->input('txtcustomer'))) {
            ...
            $results = $results->where(function ($query) use ($qry) {
                $query->whereRaw("JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".name,'$." . $this->dflang[0] . "')) like '%" . $qry . "%'")
                    ...
                    ->orwhereRaw("tblcustomers.code like '%" . $qry . "%'")
    ...
    public function status(Request $request, $id = 0)
    {
        ...
        $trash = $this->model->where('customer_id', $editid)->update(["status" => "" . $status]);
        if ($trash && $status == "yes") {
            return response()
                ->json(
                    [
