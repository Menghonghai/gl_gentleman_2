...
{
    //
    private $obj_info = ['name' => 'department', 'routing' => 'admin.controller', 'title' => 'Department', 'icon' => '<i class="text-muted fas fa-building"></i>'];
    public $args;
    ...
    private $tablename;
    private $columns = [];
    private $fprimarykey = 'department_id';
    ...
    public function __construct(array $args = [])
    {
        ...
        // dd($args['userinfo']);
        $this->obj_info['title'] =  __('dev.department');
    ...
    public function listingModel()
    {
        #DEFIND MODEL#
        return $this->model
            ->leftJoin('users', 'users.id', 'tbl_department.blongto')
            ->select(
                \DB::raw($this->fprimarykey . ",JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".name,'$." . $this->dflang[0] . "')) AS text,tbl_department.code,tbl_department.create_date,
                         tbl_department.update_date,tbl_department.status,users.name As username"),
            )->whereRaw('tbl_department.trash <> "yes"');
    } /*../function..*/
    public function listingtrash()
    {
        ...
        return $this->model
            ...
            )->whereRaw('tbl_department.trash <> "no"');
    } /*../function..*/
    ...
    public function sfp($request, $results)
    {
        ...
        $appends = [];
        $querystr = [];
        if ($request->has('txtdepartment') && !empty($request->input('txtdepartment'))) {
            $qry = $request->input('txtdepartment');
            $results = $results->where(function ($query) use ($qry) {
                ...
                ->orwhereRaw("tbl_department.code like '%" . $qry . "%'")
        ...
        if ($request->has('status') && !empty($request->input('status'))) {
            ...
            $results = $results->where("tbl_department.status", $qry);

            array_push($querystr, 'utbl_department.status=' . $qry);
            $appends = array_merge($appends, ['tbl_department.userstatus' => $qry]);
        }
    ...
    function proceed_store($request, $data, $obj_info)
    {
        ...
        // dd($save_status);
        if ($save_status) {
            //$request->file('images')->storeAs('department', $data['tableData']['image_url']);
            ...
            return response()
                ->json(
                    [
                        ...
                        "callback" => $callback,
                        "data" => [
                            $this->fprimarykey => $data['department_id'],
                            'id' => $data['department_id']
                        ]
                    ],
                    200
    ...
    public function restore(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('department_id', $editid)->update(["trash" => "no"]);
    ...
    public function totrash(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('department_id', $editid)->update(["trash" => "yes"]);
    ...
    public function disable(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('department_id', $editid)->update(["status" => "no"]);
    ...
    public function enable(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('department_id', $editid)->update(["status" => "yes"]);
    ...
    public function status(Request $request, $id = 0)
    {
        ...
        $trash = $this->model->where('department_id', $editid)->update(["status" => "" . $status]);
        if ($trash && $status == "yes") {
            return response()
                ->json(
                    [
