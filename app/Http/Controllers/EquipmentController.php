...
{
    //
    private $obj_info = ['name' => 'equipment', 'routing' => 'admin.controller', 'title' => 'Equipment', 'icon' => '<i class="text-muted fas fa-hammer"></i>'];
    public $args;
    ...
    public function default()
    {
        $equipment = $this->model
            ->select(
                ...
                DB::raw("JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".location,'$." . $this->dflang[0] . "')) AS text_location"),
        ...
        )->whereRaw('status <> "no"')->whereRaw('trash <> "yes"')->whereRaw('type <> "shop"')->get();
    ...
    public function listingModel()
    {
        ...
        return $this->model
            ...
            ->select(
                ...
                JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".location,'$." . $this->dflang[0] . "')) AS text_location,tblequipments.seria_number,tblequipments.create_date,tblequipments.update_date,tblequipments.status As status_equipment,
    ...
    public function sfp($request, $results)
    {
        ...
        if ($request->has('txtequipment') && !empty($request->input('txtequipment'))) {
            ...
            $results = $results->where(function ($query) use ($qry) {
                ...
                ->orwhereRaw("JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".location,'$." . $this->dflang[0] . "')) like '%" . $qry . "%'")
        ...
        if ($request->has('create_date') && !empty($request->input('create_date'))) {
            ...
            $date = date_query($request, 'tblequipments.create_date', 'create_date', $_date[0], last($_date));
        ...
        if ($request->has('status') && !empty($request->input('status'))) {
            ...
            array_push($querystr, 'tblequipments.status=' . $qry);
    ...
    public function setinfo($request, $isupdate = false)
    {
        ...
        $location= toTranslate($request, 'location', 0, true);
    ...
    public function edit(Request $request, $id = 0)
    {
        ...
        $input = $x;
        // select inventory if disable
        $inventory_item = $this->model_inventory
            ->select(
                \DB::raw("tblinventorys.inventory_id as id "),
                DB::raw("JSON_UNQUOTE(JSON_EXTRACT(tblinventorys.name,'$." . $this->dflang[0] . "')) AS title"),

            )->where('inventory_id', $input['inventory_id'])->get();
        $inventory_item = $inventory_item->pluck('title', 'id')->toArray();
        $inventory[array_key_first($inventory_item)] = array_values($inventory_item)[0] ?? "";
        // end
        // select vendor if disable
        $vendor_item = $this->model_vendor
            ->select(
                \DB::raw("tblvendors.vendor_id as id "),
                DB::raw("JSON_UNQUOTE(JSON_EXTRACT(tblvendors.name,'$." . $this->dflang[0] . "')) AS title"),

            )->where('vendor_id', $input['vendor_id'])->get();
        $vendor_item = $vendor_item->pluck('title', 'id')->toArray();
        $vendor[array_key_first($vendor_item)] = array_values($vendor_item)[0] ?? "";
        // end
        $name = json_decode($input['name'], true);
        $description = json_decode($input['description'], true);
        $location = json_decode($input['location'], true);
        //dd($description);
    ...
    public function detail(Request $request, $id = 0)
    {
        ...
        $inventory_item = $this->model_inventory
        ->select(
            \DB::raw("tblinventorys.inventory_id"),
            DB::raw("JSON_UNQUOTE(JSON_EXTRACT(tblinventorys.name,'$." . $this->dflang[0] . "')) AS title"),

        )->whereRaw('inventory_id= "' . $input['inventory_id'] . '"')->get();
        ...
        )->whereRaw('vendor_id= "' . $input['vendor_id'] . '"')->get();
        $vendor=$vendor_item;
        $vendor=$vendor->pluck('title','id')->toArray();
