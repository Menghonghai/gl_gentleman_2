...
{
    //
    private $obj_info = ['name' => 'inventory', 'routing' => 'admin.controller', 'title' => 'Inventory', 'icon' => '<i class="text-muted fas fa-dumpster"></i>'];
    public $args;
    ...
    public function listingModel()
    {
        ...
        return $this->model
            ...
            ->select(
                \DB::raw($this->fprimarykey . ",JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".name,'$." . $this->dflang[0] . "')) AS text,tblinventorys.code,tblinventorys.create_date,
    ...
    public function sfp($request, $results)
    {
        ...
        if ($request->has('txtinventory') && !empty($request->input('txtinventory'))) {
            ...
            $results = $results->where(function ($query) use ($qry) {
                ...
                ->orwhereRaw("tblinventorys.code like '%" . $qry . "%'")
    ...
    public function status(Request $request, $id = 0)
    {
        ...
        $trash = $this->model->where('inventory_id', $editid)->update(["status" => "" . $status]);
        if ($trash && $status == "yes") {
            return response()
                ->json(
                    [
