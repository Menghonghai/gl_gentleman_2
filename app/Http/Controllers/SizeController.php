...
{
    //
    private $obj_info = ['name' => 'size', 'routing' => 'admin.controller', 'title' => 'Size', 'icon' => '<i class="text-muted fas fa-pencil-ruler"></i>'];
    public $args;
    ...
    public function __construct(array $args = [])
    {
        ...
        // dd($args['userinfo']);
        $this->obj_info['title'] =  __('dev.size');
    ...
    public function listingModel()
    {
        #DEFIND MODEL#
        return $this->model
            ->leftJoin('users', 'users.id', 'tblsizes.blongto')
            ->select(
                \DB::raw($this->fprimarykey . ",JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".name,'$." . $this->dflang[0] . "')) AS text,tblsizes.code,tblsizes.create_date,
                         tblsizes.update_date,tblsizes.status,users.name As username"),
            )->whereRaw('tblsizes.trash <> "yes"');
    } /*../function..*/
    ...
    public function sfp($request, $results)
    {
        ...
        $appends = [];
        $querystr = [];
        if ($request->has('txtsize') && !empty($request->input('txtsize'))) {
            $qry = $request->input('txtsize');
            $results = $results->where(function ($query) use ($qry) {
                $query->whereRaw("JSON_UNQUOTE(JSON_EXTRACT(" . $this->tablename . ".name,'$." . $this->dflang[0] . "')) like '%" . $qry . "%'")
                    ->orwhereRaw("tblsizes.code like '%" . $qry . "%'")
        ...
        if ($request->has('status') && !empty($request->input('status'))) {
            ...
            $results = $results->where("tblsizes.status", $qry);

            array_push($querystr, 'tblsizes.status=' . $qry);
            $appends = array_merge($appends, ['tblsizes.userstatus' => $qry]);
        }
    ...
    function proceed_store($request, $data, $obj_info)
    {
        ...
        // dd($save_status);
        if ($save_status) {
            //$request->file('images')->storeAs('size', $data['tableData']['image_url']);
    ...
    public function restore(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('size_id', $editid)->update(["trash" => "no"]);
    ...
    public function totrash(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('size_id', $editid)->update(["trash" => "yes"]);
    ...
    public function disable(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('size_id', $editid)->update(["status" => "no"]);
    ...
    public function enable(Request $request, $id = 0)
    {
        ...
        //$routing = url_builder($obj_info['routing'], [$obj_info['name'], 'index']);
        $trash = $this->model->where('size_id', $editid)->update(["status" => "yes"]);
    ...
    public function status(Request $request, $id = 0)
    {
        ...
        $trash = $this->model->where('size_id', $editid)->update(["status" => "" . $status]);
        if ($trash && $status == "yes") {
            return response()
                ->json(
                    [
