                ...
                <div class="form-row d-flex justify-content-end" style="font-size: 11px">
                    <div class="form-group col-md-4">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control input-sm" name="txtdepartment" id="txt" placeholder="@lang('table.search_here')"
                                value="{{ request()->get('txtdepartment') ?? '' }}">
                        </div><!-- input-group -->
                    </div>
                    <div class="form-group col-md-4">
            ...
            <form name="frm-2{{ $obj_info['name'] }}" id="frm-2{{ $obj_info['name'] }}" method="POST"
                ...
                <div class="card-body table-responsive p-0 mg-t-20">
                    <table class="table  table-striped table-hover text-nowrap table-bordered">
                        ...
                        <tbody>
                            @foreach ($results as $key =>  $departments)
                                <tr>
                                    <td>
                                    ...
                                    <td style="width: 10%"> @php
                                        // dd($input);
                                        $date_array = [];
                                        $timestamp = strtotime($departments->create_date);
                                    ...
                                    <td style="width: 10%">{{ $departments->username }}</td>
                                    <td>
                                        <div class="form-group">
                                            <label class="custom-switch ps-0">
                                                <input type="checkbox" name="status" class="custom-switch-input status"
                                                    {{ $departments->status == 'yes' ? 'checked' : '' }}>
                                                    <a class="dropdown-item url"
                                                        href="{{ url_builder($obj_info['routing'], [$obj_info['name'], 'status', $departments->department_id], ['status=' . $departments->status]) }}">
                                                    </a>
                                    ...
                                    <td>
                                        @php
                                            $edit = checkpermission('departments-edit', $userinfo);
                                            $trash = checkpermission('departments-trash', $userinfo);
                                        @endphp
                                            @include('app._include.btn_record', [
                                                
                                                'rowid' => $departments->department_id,
