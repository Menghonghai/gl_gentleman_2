            ...
            $(document).ready(function() {
                ...
                $('.delete').click(function(e) {
                    ...
                    swal({
                        ...
                        cancelButtonCurrencyexchanges: 'danger',
                ...
                <div class="form-row d-flex justify-content-end" style="font-size: 11px">
                    <div class="form-group col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control input-sm" name="txtcurrencyexchange" id="txt" placeholder="@lang('table.search_here')"
                                value="{{ request()->get('txtcurrencyexchange') ?? '' }}">
                        </div><!-- input-group -->
                    </div>
                ...
                <div class="card-body table-responsive p-0 mg-t-20">
                    <table class="table  table-striped table-hover text-nowrap table-bordered">
                        ...
                        <tr style="text-align: center">
                            ...
                            <th>@lang('table.buy_price')</th>
                            <th>@lang('table.sell_price')</th>
                        ...
                        <tbody>
                            @foreach ($results as $key =>  $Currencyexchanges)
                                <tr>
                                    <td>
                                    ...
                                    <td style="width: 20%">@php
                                        // dd($input);
                                        $date_array = [];
                                        $timestamp = strtotime($Currencyexchanges->create_date);
                                    ...
                                    <td>
                                        @if (empty($Currencyexchanges->buy_in ))
                                        <justify style="color: red">(@lang('table.empty'))</justify>
                                        @else
                                        {!! num_in_khmer($Currencyexchanges->buy_in.".00") !!} @lang('table.riel')
                                        @endif
                                    </td>
                                    <td>
                                        @if (empty($Currencyexchanges->sold_out ))
                                        <justify style="color: red">(@lang('table.empty'))</justify>
                                        @else
                                        {!! num_in_khmer($Currencyexchanges->sold_out.".00") !!} @lang('table.riel')
                                        @endif
                                    </td>
                                    <td style="width: 10%">{{ $Currencyexchanges->username }}</td>
                                    <td>
                                        @php
                                            $edit = checkpermission('Currencyexchanges-edit', $userinfo);
                                            $trash = checkpermission('Currencyexchanges-trash', $userinfo);
                                        @endphp
                                            @include('app._include.btn_record', [
                                                
                                                'rowid' => $Currencyexchanges->exchange_id,
